...
/**
 * Object representing an offset. Use objects for uniqueness -
 * two identical objects represent the same offset.
 * @typedef  {object}       SourcePos
 * @property {SourcePos}    [prev]
 */
/**
 * A synthetic type, representing values in the interpreter.
 * @typedef  {object}                            InterpType
 * @property {number}                            type
 * @property {string | SourcePos | InterpType[]} [value]
 */
...
class InterpState {
    ...
    switch (bc[0]) {
      ...
      case op.RULE: {             // RULE r
        ...
        if (match > 0) {
          ...
          this.currPos = this.currPos.value
            ? {
                type: TypeTag.OFFSET,
                value: { prev: /** @type {SourcePos} */(this.currPos.value) },
              }
            : { type:TypeTag.OFFSET, value:{} };
        }
        break;
      }
  ...
  /**
   * A RULE that fails never changes currPos. If we're executing an IF_ERROR, or
   * IF_NOT_ERROR after a RULE, we can reset currPos to the value before the
   * RULE in the FAILED branch.
   *
   * @param {FormattedBytecode} bcs
   * @param {number} ip
   * @param {InterpType} top
   */
  fixCurrPosForFailed(bcs, ip, top) {
    if (InterpState.mustBe(top, TypeTag.FAILED)
        && ip && bcs[ip - 1][0] === op.RULE
        && this.currPos.value
        && typeof this.currPos.value === "object"
        && "prev" in this.currPos.value
    ) {
      this.currPos = {
        type: TypeTag.OFFSET, value: this.currPos.value.prev,
      };
    }
  }
  ...
  interpCondition(bc, ip, check) {
    ...
    if (thenType) {
      ...
      thenState.fixCurrPosForFailed(bc, ip, thenType);
    }

    if (elseType) {
      this.pop();
      this.push(elseType);
      this.fixCurrPosForFailed(bc, ip, elseType);
    }
    ...
    if (!elseSlice.length) {
      if (thenState.stack.length > this.stack.length) {
        thenSlice.push([op.POP_N, thenState.stack.length - this.stack.length]);
  ...
  /**
   * Determine whether `bc` discards the stack element at `depth` (ie removes it
   * from the stack, but ignores the value).
   *
   * @param {FormattedElement}  bc
   * @param {number}            depth
   */
  static killsStackAt(bc, depth) {
    switch (bc[0]) {
      case op.NIP:
        return depth === 1;
      case op.POP:
      ...
      case op.POP_N:
        return depth < bc[1];
      case op.CALL:
        return depth < bc[2] && bc.slice(4).every(n => n !== depth);
      case op.PLUCK:
        return depth < bc[1] && bc.slice(3).every(n => n !== depth);
      default:
        return false;
    }
  }
  ...
  /**
   * Adjust `bc` so that it works the same way after removing the stack element at
   * `depth`. Reducing bc to the empty array (eg via `bc.splice(0)`) will cause it
   * to be removed. Its an error if bc uses the element at `depth` (other than to
   * discard it).
   *
   * @param {FormattedElement}  bc
   * @param {number}            depth
   */
  static fixupForStackRemoval(bc, depth) {
    switch (bc[0]) {
      case op.NIP:
        if (depth === 1) {
          bc.splice(0);
        } else if (depth === 0) {
          throw new Error("op.NIP does not ignore the stack element at offset 0");
        }
        break;
      case op.POP:
        if (depth === 0) {
      ...
      case op.POP_N:
        ...
        break;
      case op.CALL: {
        const replacements = bc.slice(4).map(n => {
          if (n === depth) {
            throw new Error(`op.CALL does not ignore the stack element at offset ${n}`);
          }
          return n > depth ? n - 1 : n;
        });
        if (depth < bc[2]) {
          bc[2]--;
        }
        bc.splice(4);
        bc.push(...replacements);
        break;
      }
      case op.PLUCK: {
        const replacements = bc.slice(3).map(n => {
          if (n === depth) {
            throw new Error(`op.PLUCK does not ignore the stack element at offset ${n}`);
          }
          return n > depth ? n - 1 : n;
        });
        if (depth < bc[1]) {
      ...
      case op.APPEND:
        if (depth === 0) {
          throw new Error("op.APPEND does not ignore the stack element at offset 0");
        }
        if (depth === 1) {
          bc.splice(1);
          bc[0] = op.POP;
      ...
      case op.LOAD_SAVED_POS:
        if (depth === bc[1]) {
          throw new Error(`op.LOAD_SAVED_POS ${
            depth
          } does not ignore the stack element at offset ${depth}`);
        }
        if (depth < bc[1]) {
          bc[1]--;
        }
      ...
      case op.IF_LT_DYNAMIC:
      case op.IF_GE_DYNAMIC:
        if (depth === bc[1] || depth === 0) {
          throw new Error(`op ${bc[0]} ${
            depth
          } does not ignore the stack element at offset ${depth}`);
        }
        if (depth < bc[1]) {
   ...
   *
   * @param {FormattedElement} bc
   * @param {SlotCallback} inspect
   * @param {SlotCallback} pop
   * @param {SlotCallback} push
   */
  static pushPops(bc, inspect, pop, push) {
    switch (bc[0]) {
      case op.PUSH_EMPTY_STRING:  // PUSH_EMPTY_STRING
      case op.PUSH_CURR_POS:      // PUSH_CURR_POS
      case op.PUSH_UNDEFINED:     // PUSH_UNDEFINED
      ...
      case op.PLUCK: {            // PLUCK n, k, p1, ..., pK
        bc.slice(3).forEach(n => inspect(n));
        pop(bc[1]);
        push(1);
        break;
      }
      ...
      case op.CALL: {             // CALL f, n, pc, p1, p2, ..., pN
        bc.slice(4).forEach(n => inspect(n));
        pop(bc[2]);
        push(1);
        break;
      }
      ...
      default:
        throw new Error(`Invalid opcode: ${bc[0]}.`);
    }
    return false;
  }
}
