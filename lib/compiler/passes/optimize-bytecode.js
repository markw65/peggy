...
function postInterp(bc, ip, result) {
  ...
  if (result.thenState
    ...
    if (swap !== null) {
      const [thenCode, elseCode]
          = InterpState.getConditionalCode(nextBc);
      const cbc = bc[ip];
      const prevThen = /** @type {FormattedBytecode} */ (cbc[cbc.length - 2]);
      const prevElse = /** @type {FormattedBytecode} */ (cbc[cbc.length - 1]);
