...
function generateJS(ast, options) {
  ...
  function generateRuleFunction(rule) {
    ...
    function compile(bc) {
      ...
      */
      /**
        @param {(s:string,f?:boolean)=>string} condFn
        @param {number} argCount
      ...
      while (ip < end) {
        switch (bc[ip]) {
          ...
          case op.CALL: {             // CALL f, n, pc, p1, p2, ..., pN
            value = compileCall(4);
            stack.pop(bc[ip + 2]);
            const assign = stack.push(value);
            ip += 4 + bc[ip + 3];
            parts.push(isDiscard(bc[ip]) ? value + ";" : assign);
            break;
          }

          case op.RULE: {             // RULE r
            value = name(ast.rules[bc[ip + 1]].name) + "()";
            const assign = stack.push(value);
            ip += 2;
            parts.push(isDiscard(bc[ip]) ? value + ";" : assign);
            break;
          ...
          default:
            throw new Error(`Processing '${rule.name}': Invalid opcode: ${bc[ip]}.`);
        }
      }
